...
class Pipeline(PfsObject):
    ...
    def run_step_restframe(self, spec: Spectrum, **kwargs):
        ...
        if z is not None and not np.isnan(z):
            ...
            if spec.wave_edges is not None:
                spec.wave_edges = spec.wave_edges / (1 + z)
        else:
    ...
    def convolve_gaussian(self, spec, dlambda=None, vdisp=None, wlim=None):
        """
        ...
        if dlambda is not None:
            def kernel_func(w, wave):
                ...
                n = np.sum(k)
                return k / n
            res = Spectrum.convolve_vector(spec.wave, data, kernel_func, dlambda=[4 * dlambda, 4 * dlambda], wlim=wlim)
        elif vdisp is not None:
            def kernel_func(w, wave):
                # k = 1.0 / np.sqrt(2 * np.pi) / s2 ...
                z = Physics.vdisp_to_z(vdisp)
                ...
                n = np.sum(k)
                return k / n
            res = Spectrum.convolve_vector(spec.wave, data, kernel_func, dlambda=[100, 100], wlim=wlim)
        else:
